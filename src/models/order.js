    ...
    return new Promise((resolve, reject) => {
      db.query(
        `select booking.seat, id_booking, booking.id_data,profile.id_profile,movies.movie_name,category.category,cinemas.cinema_name, cinemas.cinema_room,cinemas.price,
            ...
            left join cinemas on cinemas.id_cinema = data_movies.id_room       
            left join profile on profile.id_profile = booking.id_profile            
            left join category on category.id_category = movies.id_category where profile.id_profile='${id_profile}' and booking.id_booking='${id_booking}'`,
        (err, result) => {
          if (err) {
